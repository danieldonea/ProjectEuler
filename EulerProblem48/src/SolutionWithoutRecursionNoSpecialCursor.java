public class SolutionWithoutRecursionNoSpecialCursor {
	...
	private static class EndingFiguresProcessor{
		...
		long lastFigures(int number, int nrOfFigures, int startingValue, long modolovalue){
			...
			int iteratorIndex = 0;
			while (!endingExistsFor(actualValue, iteratorIndex) && !(iterationLevel > number)){
		...
		private boolean endingExistsFor(long actualValue, int i) {
			for (i =0 ;i<=numberOfLastFigures; i++){
